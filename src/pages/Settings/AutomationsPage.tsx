        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2 items-start px-2 py-1.5 border-b border-zinc-200/60 dark:border-zinc-800/80 mb-1 select-none">
            <div className="w-1/3 min-w-[120px]">
              <span className="text-[10px] font-bold text-zinc-650 dark:text-zinc-400 block uppercase tracking-wider">Field to Populate</span>
            ...
            <div className="flex-1">
              <span className="text-[10px] font-bold text-zinc-650 dark:text-zinc-400 block uppercase tracking-wider">Value / Source Data</span>
...
export const AutomationsPage: React.FC = () => {
  ...
  const [triggerType, setTriggerType] = useState('RECORD_CREATED'); // RECORD_CREATED | RECORD_UPDATED | QUICK_ACTION | STATUS_CHANGED | ASSIGNEE_CHANGED | RELATION_LINKED | USER_MENTIONED | FORM_SUBMITTED | TIMER
  ...
  // Dynamically resolve fields for the selected local module
  const activeModule = useMemo(() => {
    return modules?.find((m) => m.id === selectedModuleId);
  }, [selectedModuleId, modules]);

  const fields = useMemo(() => {
    return activeModule ? flattenFields(activeModule.layout || []) : [];
  }, [activeModule]);
  ...
  useEffect(() => {
    if (selectedRuleId === 'new') {
      ...
      if (selectedRule.moduleId) {
        setScope('LOCAL');
        setSelectedModuleId(selectedRule.moduleId);
      } else {
        setScope('GLOBAL');
        setSelectedModuleId('');
      }
  ...
  useEffect(() => {
    if (selectedRuleId === 'new' || (selectedRule && selectedRuleId !== 'new')) {
      if (scope === 'LOCAL') {
  ...
  const handleSaveRule = async () => {
    ...
    if (scope === 'LOCAL' && !selectedModuleId) {
    ...
    if (scope === 'LOCAL') {
      ...
      } else if (triggerType === 'ASSIGNEE_CHANGED') {
        triggersPayload.push({
          type: 'ASSIGNEE_CHANGED'
        });
      } else if (triggerType === 'RELATION_LINKED') {
        triggersPayload.push({
          type: 'RELATION_LINKED',
          linkedModuleId: ''
        });
      } else if (triggerType === 'USER_MENTIONED') {
        triggersPayload.push({
          type: 'USER_MENTIONED'
        });
    ...
    const payload = {
      name,
      description,
      moduleId: scope === 'LOCAL' ? selectedModuleId : null,
      inputs: {
        visualConditions: scope === 'LOCAL' ? visualConditions : [],
        conditionMatchType: scope === 'LOCAL' ? conditionMatchType : 'AND'
      },
      actions,
      triggers: triggersPayload,
      isActive,
      conditions: finalConditions ? finalConditions.trim() : null
    };
  ...
  const filteredAutomations = useMemo(() => {
    return automations.filter((auto) => {
      ...
      if (filterScope === 'GLOBAL') {
        return matchesSearch && auto.moduleId === null;
      }
  ...
  return (
    <div className={cn(
      ...
      {selectedRuleId !== null ? (
          ...
          <aside className="w-60 flex-shrink-0 bg-zinc-55/35 dark:bg-zinc-950/20 border-r border-zinc-200 dark:border-zinc-800 p-5 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-3">
              ...
              {scope === 'LOCAL' ? (
                ...
                /* Global Scope schedule settings */
                <div className="space-y-3 bg-white dark:bg-zinc-900 border border-indigo-500/20 rounded-xl p-3 shadow-inner">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-zinc-800 dark:text-zinc-200">
                    <Calendar size={12} className="text-indigo-550 shrink-0" />
                    <span>Cron Scheduler</span>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[8px] font-bold text-zinc-400 uppercase tracking-wider">Cron Schedule</label>
                    <input 
                      type="text"
                      value={cronExpression}
                      onChange={(e) => setCronExpression(e.target.value)}
                      className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded px-2 py-1 font-mono text-[10px] text-zinc-900 dark:text-white"
                      placeholder="e.g. 0 9 * * *"
                    />
                  </div>
                  <span className="text-[7.5px] text-zinc-450 block leading-normal">Runs on schedule: `min hour day-of-month month day-of-week`.</span>
                </div>
              )}
            </div>
          ...
          <main className="flex-1 bg-zinc-50/20 dark:bg-zinc-950/10 p-8 overflow-y-auto custom-scrollbar flex flex-col gap-6 min-w-0">
            ...
            <div className="flex-1 space-y-4">
              {actions.map((action, idx) => (
                <div 
                  key={idx}
                  className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm animate-in fade-in duration-200"
                >
                  <div 
                          ...
                          <div className="space-y-2">
                            ...
                            {fieldsEditorModes[idx] === 'json' ? (
                              ...
                              <VisualFieldsMapper
                                ...
                                triggerModuleId={selectedModuleId}
                                actionIdx={idx}
                          ...
                          <div className="space-y-2">
                            ...
                            {fieldsEditorModes[idx] === 'json' ? (
                              ...
                              <VisualFieldsMapper
                                ...
                                targetModuleId={action.config.targetType === 'TRIGGERING' ? (selectedModuleId || undefined) : undefined}
                                triggerModuleId={selectedModuleId}
                                actionIdx={idx}
                                triggerFields={fields}
                                modules={modules || []}
                        ...
                        <div className="space-y-3 text-left">
                          <div className="space-y-1">
                            ...
                            >
                              ...
                              {modules?.filter((m: any) => m.id !== selectedModuleId).map((m: any) => (
                                <option key={m.id} value={m.id}>{m.name}</option>
                              ))}
                            </select>
                          </div>
              ...
              {actions.length === 0 && (
                <div className="py-20 text-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl flex flex-col items-center justify-center gap-3">
                  <Play size={24} className="text-zinc-350 dark:text-zinc-650" />
          ...
          <aside className="w-80 flex-shrink-0 bg-white dark:bg-zinc-950 border-l border-zinc-200 dark:border-zinc-800 p-5 flex flex-col gap-5 overflow-y-auto custom-scrollbar">
            ...
            {rightSidebarTab === 'properties' ? (
              <div className="space-y-6 flex-1">
                ...
                {/* Scope selector */}
                <div className="space-y-3">
                  <h4 className="text-[9px] font-black text-zinc-450 dark:text-zinc-500 uppercase tracking-widest px-1">Scope</h4>
                  
                  <div className="grid grid-cols-2 gap-2 bg-zinc-55/35 dark:bg-zinc-900/10 p-2 border border-zinc-200 dark:border-zinc-800 rounded-xl">
                    <button
                      type="button"
                      onClick={() => setScope('LOCAL')}
                      className={cn(
                        "py-1 rounded-lg text-[9.5px] font-bold transition-all cursor-pointer flex items-center justify-center gap-1",
                        scope === 'LOCAL'
                          ? "bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white shadow-sm"
                  ...
                  {scope === 'LOCAL' && (
                    <div className="space-y-1 mt-2">
                      <label className="text-[9.5px] font-bold text-zinc-500 px-1">Module Context</label>
                      <select
                        value={selectedModuleId}
                        onChange={(e) => setSelectedModuleId(e.target.value)}
                        className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-2 py-1.5 text-xs text-zinc-900 dark:text-white cursor-pointer"
                      >
                        {modules?.map(m => (
                          <option key={m.id} value={m.id}>{m.name}</option>
                        ))}
              ...
              <div className="flex-grow flex flex-col gap-4 min-h-0">
                ...
                <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                  {loadingRuns ? (
                    ...
                    <p className="text-[10.5px] text-zinc-450 italic text-center py-8">No execution history found.</p>
                  ) : (
                    runs.map((run) => (
                      <div 
                        key={run.id}
        ...
        <div className="flex-1 flex items-center justify-center bg-zinc-50/20 dark:bg-zinc-950/10 animate-in fade-in duration-200">
