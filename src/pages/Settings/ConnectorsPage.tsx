...
export const ConnectorsPage = () => {
  ...
  const cacheKey = `connectors_${tenant?.id || 'default'}`;
  const [registry, setRegistry] = useState<Connector[]>(() => builderCache.get<Connector[]>(cacheKey) || DEFAULT_CONNECTORS);
  const [activeConnectors, setActiveConnectors] = useState<TenantConnector[]>(() => builderCache.get<TenantConnector[]>(`${cacheKey}_active`) || []);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const nextRegistry = data.registry && data.registry.length > 0 ? data.registry : DEFAULT_CONNECTORS;
        setRegistry(nextRegistry);
        builderCache.set(cacheKey, nextRegistry);
        if (data.active) {
          setActiveConnectors(data.active);
          builderCache.set(`${cacheKey}_active`, data.active);
        }
      } else {
        setRegistry(DEFAULT_CONNECTORS);
        builderCache.set(cacheKey, DEFAULT_CONNECTORS);
      }
    } catch (err) {
      ...
      <div className="flex-1 px-6 lg:px-12 pt-8 pb-20 relative z-10 space-y-6">
        {!selectedConnectorId ? (
          <>
            ...
            {loading ? null : filteredRegistry.length === 0 ? (
              <EmptyState
                icon={Plug}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredRegistry.map((conn, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="group border border-white/20 dark:border-white/5 bg-white/40 dark:bg-white/[0.03] backdrop-blur-xl rounded-3xl p-6 hover:border-indigo-500/50 dark:hover:border-indigo-500/50 transition-[border-color,box-shadow,background-color] duration-200 shadow-xl shadow-black/5 dark:shadow-none hover:shadow-indigo-500/10 cursor-pointer flex flex-col justify-between h-full relative overflow-hidden min-h-[220px]"
                    >
                      ...
                      <div className="relative z-10 flex flex-col h-full justify-between">
                        <div>
                          ...
                          <h3 className="text-base font-bold text-zinc-900 dark:text-white group-hover:text-indigo-500 transition-colors duration-150">{conn.name}</h3>
                ...
                <motion.div
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.25, delay: filteredRegistry.length * 0.03, ease: 'easeOut' }}
                  onClick={() => setIsModalOpen(true)}
